class OriginsController < ApplicationController
  ...
  # GET /origins.json
  def index
    @origins = Origin.all.reverse_order
    @count = Origin.count
  end
  ...
  def create
    ...
    respond_to do |format|
      if @origin.save
        format.html { redirect_to containers_path, notice: 'Origin was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @origin.update(origin_params)
        format.html { redirect_to origins_path, notice: 'Origin was successfully updated.' }
  ...
  private
    ...
    def origin_params
      params.require(:origin).permit(:location)
    end
